...
""" 
Note: for optimal usage, use each and every function individually. some of the 
functions are nested in one another and therefore defaults input parameters are 
taken.
"""

#%% 
def qt(df, u2=False, a=0.8):
    """
    ...
    try:
        df["Rf"] = (df.fs / (df.qt)) * 100            # Recalculate friction ratio [in percentage]
    except:
        print("Rf was not recalculated due to error (most likely because friction sleeve ratio was not present")
...
def sig_eff(df, water_table=1,sea_level=False):
    ...
    if sea_level:
        """Include sea level into effective stress calculation"""
...
def Isbt(df):
    ...
    # df.Isbt.iloc[0:5] = np.nan      # =nan as takes a while for friction readings to catch up
    # df.Isbt.iloc[-5:] = np.nan      # idem
...
if __name__ == "__main__":
    folder = "C:\\Users\\kduffy\\OneDrive\\Education\\University\\03_PhD\\BB_Data\\23_Delft Pile Test\\cpts\\GEF\\"
    file = "CPTU1_1.gef"
    import matplotlib.pyplot as plt
    g=GEF()
    g.readFile(folder + file)
    df = g.asDataFrame()
    which = "baldi et al_1986"
    df = relative_density(df,which=which)
    plt.plot(df.Dr,df.z,label=which)
    
    g=GEF()
    g.readFile(folder + file)
    df = g.asDataFrame()
    which = "kulhawy & mayne_1990_simplified"
    df = relative_density(df,which=which)
    plt.plot(df.Dr,df.z,label=which)
    ...
    g.readFile(folder + file)
    df = g.asDataFrame()
    which = "jamiolkowski_2003_saturated"
    df = relative_density(df,which=which)
    plt.plot(df.Dr,df.z,label=which)
    plt.legend()