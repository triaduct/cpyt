...
if __name__ == "__main__":
    # dataFolder = "C:\\Users\\kduffy\\Downloads\\cpts\\"
    dataFolder = phdDir + "BB_Data\\mv2 pile test\\site investigation\\test site_gef files\\"
    # targetFolder = phdDir + "\\delete\\"
    file = "I.001067_E1.gef"
    # file = "CPTU1_0.gef"
    ...
    for file in os.listdir(dataFolder):
        if file.split(".")[-1].lower() == "gef":
            ...
            df = g.asDataFrame()
            try:
                df = df.loc[(df.z < upper[g.cpt]) & (df.z > lower[g.cpt])]
                qc_avg.append(df.qc.mean())
                cov.append(np.std(df.qc)/np.mean(df.qc))
            except:
                continue

    print(np.nanmean(cov))
    print(np.nanmean(qc_avg))

    
    # dataFolder = phdDir + "BB_Data\\60_Rdam CPTs_with u2\\"
    # getCoords(dataFolder)
    # dataFolder = phdDir + "BB_Data\\99_Other\\CPTs Deltares\\"
    
    # df=getCoords(dataFolder)
    # for file in os.listdir(dataFolder)[::40]:
    #     g=GEF()
    #     g.readFile(dataFolder + file)
    #     plt.plot(g.qc,g.dz,label=g.cpt)
    #     plt.legend()
    #     plt.xlabel("Cone resistance")
    #     plt.ylabel("Depth [m[")
        # gefToExcel(dataFolder+file,targetFolder)
